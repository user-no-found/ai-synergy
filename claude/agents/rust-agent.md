...
```
- 【启动时记忆管理】必须先检查并读取/创建 Record/Memory/rust-agent-{nn}.md
...
4. 检查 Record/Memory/rust-agent-{nn}.md 是否存在：
...
```
- 首个实例：rust-agent-01
- 多实例时按启动顺序递增：rust-agent-02、rust-agent-03
...
4. 更新 Record/Memory/rust-agent-{nn}.md：
   - 新增文件 → 添加到"实现记录"
   - 完成功能 → 更新"当前状态"
5. 输出完成报告
```
